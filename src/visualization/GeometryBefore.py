...
if __name__ == "__main__":
    ...
    for j in range(0, numberWells):
        ...
        if typeWell[4 * j][0] == 1:    
            g = plt.scatter(graphsx[j], graphsy[j], marker="o", s = 350)        
            plt.setp(g, color=colors[j]) 
        else :   
            g = plt.scatter(graphsx[j], graphsy[j], marker="^", s = 350)        
            plt.setp(g, color=colors[j]) 
    ...
    plt.savefig('../../res/graph/Geometry before.png', bbox_inches='tight')            
    plt.show() 